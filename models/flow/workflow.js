...
var stack = require('./stack.js');//并没用到
...
function wsapworkflow() {
    ...
    this.switchState = function (varResult) {
        ...
        logFile.log("status switchState varResult: " + varResult.toString());
        ...
        while (true) {
            ...
            if (varNodes.length == 0) {
                ...
                logFile.log('next node not exist,varXPath:' + varXPath + ',current node:' + this.runNode[0]);
                return false;
            }
            ...
            if (varSize <= 0) {
                ...
                logFile.log(this.varStateFile + " finished.");

                //
                process.send("SingleFinished");
                // changeStatus.removeAllListeners();
                return false;
            }

            if (this.stackReWind() == false) {
                console.log('this.stackReWind() failed');
                logFile.log('this.stackReWind() failed');
                return false;
            }
        ...
        global.nodeName = this.getNodeName();
        ...
        while (true) {
            ...
            if (varProperty == 'logic') {
                ...
                logFile.log('this.switchSubFlow:' + varSubFlowName);
                if (!this.switchSubFlow(varSubFlowName)) {
                    console.log('this.switchSubFlow:' + varSubFlowName + 'failed');
                    logFile.log('this.switchSubFlow:' + varSubFlowName + 'failed');
                    return false;
                }
            }
            else {
    ...
    /*
     ...
     bService-是否为服务
     */
    this.loadWorkFlow = function (varName, bService) {
        try {
    ...
    this.switchSubFlow = function (varSubName) {
        try {
            ...
            logFile.log("status switchSubFlow: " + varSubName.toString());
            ...
            //状态点迁移
            var varXPath = '/statechart/state[@id=\'' + this.flowName.toString() + '\'][@property=\'sub\']';
    ...
    this.resetToRootNode = function (varRootNodeName) {
